...
#creo listas con categorias y los len de cada categoria
categorias = list(zip(["peliculas_accion", "peliculas_comedia", "peliculas_drama"], [len(peliculas_accion), len(peliculas_comedia), len(peliculas_drama)]))

categoria_mas_pop = max(categorias, key=lambda x: x[1])[0]
print("el genero mas popular es:", categoria_mas_pop)
